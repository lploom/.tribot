...
public class CommonUtils {
   ...
   public boolean clickModel(RSModel model, String upText) {
      ...
      if(upText.contains("->")) model.click();
      
      Point p = model.getCentrePoint();
      p.setLocation(p.x + General.random(-7, 7), p.y + General.random(-7, 7));
      Mouse.hop(p);
      if (!cam.isCameraPitching() && !cam.isCameraRotating()
            && Timing.waitUptext(upText, General.random(350, 400))) {
         Mouse.click(1);
         ...
         while (System.currentTimeMillis() - failsafe < 1000) {
            ...
            if (color.getRed() > 240 && color.getGreen() < 10
                  && color.getBlue() < 10) {
               return true;
            }
            // check if click was YELLOW
            else if (color.getRed() > 245 && color.getGreen() > 245
                  && color.getBlue() < 10) {
               return false;
            }
            sleep(10, 20);
   ...
   public boolean dropJunk(int[] junk) {
