...
public class CowHideKiller extends Script implements Painting, RandomEvents, Ending, Breaking {
  ...
  private void banking(int bankLocation) { //TODO BANKING ENUM
    if (bankLocation == 0) {
      if (!inLumbyBank()) {
        moveToBanktile();
      }
  ...
  private boolean isTeleportRandom(RANDOM_SOLVERS r) {
    return r == RANDOM_SOLVERS.APPENDAGE || r == RANDOM_SOLVERS.BEEKEEPER || r == RANDOM_SOLVERS.CAPTARNAV || 
        ...
        r == RANDOM_SOLVERS.GRAVEDIGGER|| r == RANDOM_SOLVERS.MAZE || r == RANDOM_SOLVERS.MIME || r == RANDOM_SOLVERS.MOLLY ||
        r == RANDOM_SOLVERS.MORDAUT || r == RANDOM_SOLVERS.PILLORY || r == RANDOM_SOLVERS.PINBALL || r == RANDOM_SOLVERS.PRISONPETE ||
  ...
  private State getState() { // TODO BANKING ENUM
    if(Game.getCurrentWorld() != startingWorld) {
      ...
      if (((bankLocation == 0 || bankLocation == 1) && Inventory.getAll().length == 0)
