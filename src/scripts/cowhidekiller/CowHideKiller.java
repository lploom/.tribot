...
public class CowHideKiller extends Script implements Painting, RandomEvents, Ending, Breaking {
   ...
   private final static RSTile[] PATH_TO_COWS = { new RSTile(3213, 3214),
      new RSTile(3216, 3218), new RSTile(3222, 3218),
      new RSTile(3235, 3220), new RSTile(3245, 3225),
      new RSTile(3254, 3225), new RSTile(3259, 3232),
      new RSTile(3259, 3239), new RSTile(3255, 3246),
      new RSTile(3251, 3252), new RSTile(3250, 3258),
      new RSTile(3250, 3266), };
   private final static RSTile[] PATH_TO_STAIRS = { new RSTile(3250, 3265),
      new RSTile(3251, 3254), new RSTile(3260, 3245),
      new RSTile(3260, 3238), new RSTile(3260, 3231),
      new RSTile(3254, 3226), new RSTile(3245, 3226),
      new RSTile(3236, 3226), new RSTile(3234, 3218),
   ...
   private final static RSTile[] PATH_TO_AK_GATE = { new RSTile(3268, 3227),
      new RSTile(3250, 3265), new RSTile(3251, 3254),
      new RSTile(3260, 3245), new RSTile(3260, 3238),
      new RSTile(3260, 3231), new RSTile(3266, 3227) };
   private final static RSTile[] PATH_TO_AK_BANK = { new RSTile(3268, 3227),
      new RSTile(3271, 3219), new RSTile(3272, 3213),
      new RSTile(3274, 3208), new RSTile(3277, 3201),
      new RSTile(3280, 3196), new RSTile(3282, 3191),
      new RSTile(3282, 3185), new RSTile(3278, 3180),
   ...
   CommonUtils commonUtils = new CommonUtils(cameraUtils);
   KMUtils utils = new KMUtils(cameraUtils, commonUtils);
   ...
   public void run() {
      ...
      while (isRunning) {
         calcPaint();
         state = getState();
         plane = Player.getPosition().getPlane();
         ...
         switch (state) {
            case BANKING:
               ...
               break;
            case KILLING_COWS:
               if (selectedCombatStyleIndex != Combat.getSelectedStyleIndex()) {
      ...
      while (Inventory.getCount(gear) > 0) {
         ...
         if (items != null && items.length > 0) {
            for (RSItem item : items) {
               item.click("Wield", "Wear");
               sleep(1500, 2500);
            }
         }
      }
      while (selectedCombatStyleIndex != Combat.getSelectedStyleIndex()) {
         Combat.selectIndex(selectedCombatStyleIndex);
         sleep(1500, 2500);
      }
      if (!utils.isAutoRetaliateIsEnabled()) {
         utils.setAutoRetaliate(true);
         sleep(1500, 2500);
      }
      if (!GameTab.getOpen().equals(TABS.INVENTORY)) {
         GameTab.open(TABS.INVENTORY);
   ...
   private void killTarget(RSNPC target, int deathAnimationID, int minHp, boolean getLoot) {
      if (Player.getPosition().distanceTo(target) > 7) {
         ...
         if (commonUtils.clickModel(target.getModel(), "Attack")) {
            ...
            // move mouse or camera after attacking target
            int antibanIndex = General.random(0, 4);
            if (antibanIndex < 3) {
               Mouse.move(General.random(-1000, 1000),
                     General.random(-1000, 1000));
            }
   ...
   private void moveToCows() {
      if (plane > 0) {
         // antiban toggle run on different floors
   ...
   private void travelToBank(int bankLocation) {
      if (plane == 0) {
         if (atCows()) {
            ...
            else if (getDistanceToTrapdoor() < 15
                  && getDistanceToTrapdoor() >= 7) {
               if (!Player.isMoving()) {
   ...
   private void doBank(int bankLocation) {
      ...
      if (Inventory.getCount() != 0 || (bankLocation == 2 && !donePrinceAliQuest && Inventory.getCount(COINS_ID) < 20)) {
   ...
   private void travelToCows() {
      if (!atGate()) {
         ...
         if (bankLocation == -1 || bankLocation == 0 || bankLocation == 1|| (bankLocation == 2 && !inAlKharid())) {
            if(GATE_TILE_OUTSIDE.getY()+3 > Player.getPosition().getY()) { // failsafe if north of the gate
   ...
   private void openGate() {
      int rand = General.random(0, 1);
      Positionable gateTile;
      if(rand == 0) {
   ...
   private void climbStairs(int plane) {
      ...
      if(stairs != null) { 
         ...
         if (utils.rightClickOnModel(stairs.getModel(), (s + " Staircase"))) {
            if (plane == 0) {
   ...
   private RSNPC findNearestTarget() {
      if(Player.getRSPlayer().getInteractingCharacter() != null) {
         RSNPC target = (RSNPC) Player.getRSPlayer().getInteractingCharacter();
   ...
   private State getState() {
      if(Game.getCurrentWorld() != startingWorld) {
         ...
         if (((bankLocation == 0 || bankLocation == 1) && Inventory.getCount() == 0)
               || (bankLocation == 2 && Inventory.getCount(COINS_ID) >= 20 && !donePrinceAliQuest && Inventory.getCount(COWHIDE_ID) == 0)
               || (bankLocation == 2 && donePrinceAliQuest && Inventory.getCount() == 0) 
               || bankLocation == -1) {
   ...
   public void onRandom(RANDOM_SOLVERS r) {
      if(r == RANDOM_SOLVERS.SECURITYGUARD || r == RANDOM_SOLVERS.STRANGEPLANT) return;
      ...
      while(Player.getRSPlayer().isInCombat()) {
         if(System.currentTimeMillis() - k > 5000) break;
         sleep(100, 200);
      }
